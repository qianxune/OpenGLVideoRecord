                    ...
                    if (input != null) {
                        try {
                            ...
                            if (inputBufferIndex >= 0) {
                                ...
                                mediaCodec.queueInputBuffer(inputBufferIndex, 0, input.length, System.currentTimeMillis(), 0);//输入流入队列
                                generateIndex += 1;
                            }

                            MediaCodec.BufferInfo bufferInfo = new MediaCodec.BufferInfo();
                            int outputBufferIndex = mediaCodec.dequeueOutputBuffer(bufferInfo, TIMEOUT_USEC);//从输出队列中取出编码操作之后的数据
                            while (outputBufferIndex >= 0) {
                                ...
                                if (bufferInfo.flags == MediaCodec.BUFFER_FLAG_CODEC_CONFIG) {
                                    ...
                                    outputStream.write(outData, 0, outData.length);//写入文件
                                }

                                mediaCodec.releaseOutputBuffer(outputBufferIndex, false);//处理完成，释放ByteBuffer数据
